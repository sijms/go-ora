...
type AdvNegoService interface {
	...
	writeServiceData(session *network.Session) error
	readServiceData(session *network.Session, subPacketNum int) error
...
func (serv *defaultService) writePacketHeader(session *network.Session, length, _type int) error {
...
func (serv *defaultService) writeHeader(session *network.Session, serviceSubPackets int) {
	session.PutInt(serv.serviceType, 2, true, false)
	session.PutInt(serviceSubPackets, 2, true, false)
	session.PutInt(0, 4, true, false)
}
...
func (serv *defaultService) writeVersion(session *network.Session) error {
	err := serv.writePacketHeader(session, 4, 5)
	if err != nil {
		return err
	}
